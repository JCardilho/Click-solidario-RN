...
export const DefaultInformationsForSoliciteDonationsPage = () => {
  const [show, setShow] = useState<boolean>();

  const handleShowInCache = async () => {
    try {
      const cache = await AsyncStorage.getItem('showDefaultInformationsForSoliciteDonationsPage');
  ...
  const handleHiddenInCache = async () => {
    try {
      await AsyncStorage.setItem('showDefaultInformationsForSoliciteDonationsPage', 'false');
      setShow(false);
    } catch (err) {
      console.error(err);
    }
      ...
      {show === true && (
        <View className="w-full bg-yellow-100/100 p-2 border border-yellow-400 rounded-lg mb-4">
          <Text className="text-xl font-kanit text-center">
            Utilize essa tela para solicitar ou doar itens
          </Text>
