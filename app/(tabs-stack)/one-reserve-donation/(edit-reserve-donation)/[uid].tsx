...
export default function EditOneReserveDonation() {
  ...
  const { data, refetch } = useQuery<IReserveDonation>({
    ...
    queryFn: async () => {
      ...
      if (searchForData) {
        ...
        setValue('image', searchForData.images || ([] as any));
      ...
      setImages(result.images || ([] as any), result.uid);
      setIsLoading(false);
    ...
    mutationFn: async () => {
      ...
      if (!result.canceled) {
        ...
        result.assets.map(async (image) => {
          const response = await ReserveDonationsService.AddImages(data?.uid, image.uri);
          console.log('response link', response);
          if (!response) return;
          setImages(getValues('image') ? [...getValues('image'), response] : [response], data?.uid);
  ...
  const { mutate: removeImage } = useMutation({
    ...
    mutationFn: async (index: number) => {
      ...
      if (!data?.uid) return;
      await ReserveDonationsService.DeleteImage(images[index], data?.uid);
  ...
  const { mutate: mutateAddImage, isPending: isPendingAddImage } = useMutation({
    mutationKey: ['update-reserve-donation', uid],
    mutationFn: async (data: IReserveDonation) => {
      const result = await ReserveDonationsService.UpdateReserveDonation(data.uid, data);
      return result;
    },
    onSuccess: (data) => {
